...
class QuotesSpider(scrapy.Spider):
    name = "quotes"
    custom_settings = {
        'LOG_FILE': 'E:/گزارشات لاگ ها/Economist/BourseViewJapan.txt',
    }

    def start_requests(self):
        urls = [
            'https://www.tradingview.com/markets/stocks-japan/market-movers-highest-net-income/'
        ]
        for url in urls:
    ...
    def parse(self, response):
        ...
        print(dd)
        dd.to_csv("E:\\PY\Economist\\CSVs\\BourseViewJapan.csv", encoding='utf-8-sig')
        main()
def main():
    try:
        import logging
        logging.basicConfig(filename='E:/گزارشات لاگ ها/Economist/BourseViewJapan.txt', level=logging.DEBUG,
        ...
        result = pd.DataFrame();
        finalResult = pd.DataFrame();
        dd = pd.read_csv("E:\\PY\Economist\\CSVs\\BourseViewJapan.csv")
        first = True
        ...
        conn = pyodbc.connect('Driver={SQL Server};'
                                  ...
                                  'UID:sa;'
                                  'PWD:Adib4626')
        # data1.to_sql("BoursViewJapan", conn, if_exists='append' )
        cursor = conn.cursor()
        print('insert into sql')
        for row in data1.itertuples():
            print(row)
            cursor.execute('''INSERT INTO Adib.dbo.BoursViewJapan(Symbol , [Net income(FY)] , [Price] , [Change % 1D] ,
     [Volume 1D], [Market cap], [P/E], [EPS(TTM)], [Sector] )
        ...
        sqlHistory = pandas.read_sql_query('select * from [dbo].[VBoursViewJapan]', conn)
        sqlHistory.to_excel('E:\\گزارشات اکسل\\Economist\\BourseViewJapan.xlsx', index=False)
