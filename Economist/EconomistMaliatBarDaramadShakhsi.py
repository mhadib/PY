...
class QuotesSpider(scrapy.Spider):
    name = "quotes"
    custom_settings = {
        'LOG_FILE': 'E:/گزارشات لاگ ها/Economist/EconomistMaliatBarDaramadShakhsi.txt',
    }
    ...
    def parse(self, response):
        try:
            import logging
            logging.basicConfig(filename='E:/گزارشات لاگ ها/Economist/EconomistMaliatBarDaramadShakhsi.txt', level=logging.DEBUG,
            ...
            print(dd)
            dd.to_csv("E:\\PY\Economist\\CSVs\\MaliatBarDaramadShakhsi.csv", encoding='utf-8-sig')
            main()
...
def main():
    ...
    dd = pd.read_csv("E:\\PY\Economist\\CSVs\\MaliatBarDaramadShakhsi.csv")
    ...
    for row in finalResult.itertuples():
        cursor.execute('''INSERT INTO Adib.dbo.MaliatBarDaramadShakhsi([Keshvar] ,[Gozashte])
                                                   VALUES (?,?)
                                                     ''',
                           row[1],
                           row[2])
    ...
    sqlHistory = pd.read_sql_query('select * from vMaliatBarDaramadShakhsi			', conn)
    sqlHistory.to_excel('E:\\گزارشات اکسل\\Economist\\vMaliatBarDaramadShakhsi.xlsx', index=False)
    print("main done")
#main()