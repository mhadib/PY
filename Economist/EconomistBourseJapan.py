...
class QuotesSpider(scrapy.Spider):
    name = "quotes"
    custom_settings = {
        'LOG_FILE': 'E:/گزارشات لاگ ها/Economist/EconomistBourseJapan.txt',
    }
    def start_requests(self):
        urls = [
            'https://fa.tradingeconomics.com/japan/stock-market'
        ]
        for url in urls:
    ...
    def parse(self, response):
        ...
        print(dd)
        dd.to_csv("E:\\PY\Economist\\CSVs\\bourseJapan.csv", encoding='utf-8-sig')
        main()
def main():
    try:
        import logging
        logging.basicConfig(filename='E:/گزارشات لاگ ها/Economist/EconomistBourseJapan.txt', level=logging.DEBUG,
        ...
        result = pd.DataFrame();
        finalResult = pd.DataFrame();
        dd = pd.read_csv("E:\\PY\Economist\\CSVs\\bourseJapan.csv")
        first = True
        ...
        for row in finalResult.itertuples():
            print(row)
            cursor.execute('''INSERT INTO Adib.dbo.BoursJapan(Name,FullName,Gheymat,IndexChange,DayDarsad,YearDarsad,RDate)
                                                       VALUES (?,?,?,?,?,?,?)
                                                         ''',
                               row[1],
                               row[2],
        ...
        # cursor.execute('truncate table VBoursJapan; insert into VBoursJapan select * from VBoursJapan_PreProcess order by [درصد تغییر نسبت به سال 2005] desc ')
        # conn.commit()
        #
        sqlHistory = pandas.read_sql_query('select * from [dbo].[VBoursJapan] order by [درصد تغییر نسبت به سال 2005] desc	', conn)
        sqlHistory.to_excel('E:\\گزارشات اکسل\\Economist\\VBoursJapan.xlsx', index=False)
