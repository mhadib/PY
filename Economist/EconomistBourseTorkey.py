...
class QuotesSpider(scrapy.Spider):
    name = "quotes"
    custom_settings = {
        'LOG_FILE': 'E:/گزارشات لاگ ها/Economist/EconomistBourseTorkey.txt',
    }

    def start_requests(self):
        urls = [
            'https://fa.tradingeconomics.com/turkey/stock-market'
        ]
        for url in urls:
    ...
    def parse(self, response):
        ...
        print(dd)
        dd.to_csv("E:\\PY\Economist\\CSVs\\bourseTorkey.csv", encoding='utf-8-sig')
        main()
def main():
    try:
        import logging
        logging.basicConfig(filename='E:/گزارشات لاگ ها/Economist/EconomistBourseTorkey.txt', level=logging.DEBUG,
        ...
        result = pd.DataFrame();
        finalResult = pd.DataFrame();
        dd = pd.read_csv("E:\\PY\Economist\\CSVs\\bourseTorkey.csv")
        first = True
        ...
        for row in finalResult.itertuples():
            print(row)
            cursor.execute('''INSERT INTO Adib.dbo.BoursTorkey(Name,FullName,Gheymat,IndexChange,DayDarsad,YearDarsad,RDate)
                                                       VALUES (?,?,?,?,?,?,?)
                                                         ''',
                               row[1],
                               row[2],
        ...
        conn.commit()
        #
        sqlHistory = pandas.read_sql_query('select * from [dbo].[VBoursTorkey]	order by [درصد تغییر نسبت به سال 2005] desc	', conn)
        sqlHistory.to_excel('E:\\گزارشات اکسل\\Economist\\VBoursTorkey.xlsx', index=False)
