...
class QuotesSpider(scrapy.Spider):
    name = "quotes"
    custom_settings = {
        'LOG_FILE': 'E:/گزارشات لاگ ها/Economist/EconomistTolidKhodro.txt',
    }
    def start_requests(self):
        urls = [
            'https://fa.tradingeconomics.com/country-list/car-production'
        ]
        for url in urls:
    ...
    def parse(self, response):
        try:
            import logging
            logging.basicConfig(filename='E:/گزارشات لاگ ها/Economist/EconomistTolidKhodro.txt', level=logging.DEBUG,
            ...
            print(dd)
            dd.to_csv("E:\\PY\Economist\\CSVs\\TolidKhodro.csv", encoding='utf-8-sig')
            main()
...
def main():
    ...
    dd = pd.read_csv("E:\\PY\Economist\\CSVs\\TolidKhodro.csv")
    ...
    for row in finalResult.itertuples():
        print(row)
        cursor.execute('''INSERT INTO Adib.dbo.TolidKhodro([Keshvar] ,[Gozashte],[Vahed])
                                                   VALUES (?,?,?)
                                                     ''',
                           row[1],
                           row[2],
                           row[3])
    conn.commit()
    sqlHistory = pd.read_sql_query('select * from vTolidKhodro						', conn)
    sqlHistory.to_excel('E:\\گزارشات اکسل\\Economist\\vTolidKhodro.xlsx', index=False)
    print("main done")
# main()