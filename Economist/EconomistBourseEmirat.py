...
class QuotesSpider(scrapy.Spider):
    name = "quotes"
    custom_settings = {
        'LOG_FILE': 'E:/گزارشات لاگ ها/Economist/EconomistBourseEmirat.txt',
    }

    def start_requests(self):
        urls = [
            'https://fa.tradingeconomics.com/united-arab-emirates/stock-market'
        ]
        for url in urls:
    ...
    def parse(self, response):
        try:
            import logging
            logging.basicConfig(filename='E:/گزارشات لاگ ها/Economist/EconomistBourseEmirat.txt', level=logging.DEBUG,
            ...
            print(dd)
            dd.to_csv("E:\\PY\Economist\\CSVs\\emirat.csv", encoding='utf-8-sig')
            main()
...
def main():
    ...
    dd = pd.read_csv("E:\\PY\Economist\\CSVs\\emirat.csv")
    first = True
    ...
    for row in finalResult.itertuples():
        print(row)
        cursor.execute('''INSERT INTO Adib.dbo.BoursEmirat(Name,FullName,Gheymat,IndexChange,DayDarsad,YearDarsad,RDate)
                                                   VALUES (?,?,?,?,?,?,?)
                                                     ''',
                           row[1],
                           row[2],
    ...
    conn.commit()
    #
    sqlHistory = pandas.read_sql_query('select * from [dbo].[VBoursEmirat]		order by [درصد تغییر نسبت به سال 2005] desc			', conn)
    sqlHistory.to_excel('E:\\گزارشات اکسل\\Economist\\VBoursEmirat.xlsx', index=False)
