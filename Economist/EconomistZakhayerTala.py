...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = [
            'https://fa.tradingeconomics.com/country-list/gold-reserves'
        ]
        for url in urls:
    ...
    def parse(self, response):
        try:
            import logging
            logging.basicConfig(filename='E:/گزارشات لاگ ها/Economist/ZakhayerTala.txt', level=logging.DEBUG,
            ...
            print(dd)
            dd.to_csv("E:\\PY\Economist\\CSVs\\ZakhayerTala.csv", encoding='utf-8-sig')
            main()
...
def main():
    result = pd.DataFrame();
    finalResult = pd.DataFrame();
    dd = pd.read_csv("E:\\PY\Economist\\CSVs\\ZakhayerTala.csv")
    ...
    for row in finalResult.itertuples():
        cursor.execute('''INSERT INTO Adib.dbo.ZakhayerTala([Keshvar] ,[Gozashte])
                                                   VALUES (?,?)
                                                     ''',
                           row[1],
                           row[2])
    ...
    sqlHistory = pd.read_sql_query('select * from vZakhayerTala						', conn)
    sqlHistory.to_excel('E:\\گزارشات اکسل\\Economist\\vZakhayerTala.xlsx', index=False)
