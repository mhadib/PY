...
class QuotesSpider(scrapy.Spider):
    name = "quotes"
    custom_settings = {
        'LOG_FILE': 'E:/گزارشات لاگ ها/Economist/BourseViewUSA.txt',
    }
    ...
    def parse(self, response):
        ...
        print(dd)
        dd.to_csv("E:\\PY\Economist\\CSVs\\BourseViewUSA.csv", encoding='utf-8-sig')
        main()
def main():
    ...
    try:
        import logging
        logging.basicConfig(filename='E:/گزارشات لاگ ها/Economist/BourseViewUSA.txt', level=logging.DEBUG,
        ...
        result = pd.DataFrame()
        finalResult = pd.DataFrame()
        dd = pd.read_csv("E:\\PY\Economist\\CSVs\\BourseViewUSA.csv")
        first = True
        ...
        for row in data1.itertuples():
            print(row)
            cursor.execute('''INSERT INTO Adib.dbo.BoursViewUSA(Symbol , [Net income(FY)] , [Price] , [Change % 1D] ,
     [Volume 1D], [Market cap], [P/E], [EPS(TTM)], [Sector] )
        ...
        sqlHistory = pandas.read_sql_query('select * from [dbo].[VBoursViewUSA]', conn)
        sqlHistory.to_excel('E:\\گزارشات اکسل\\Economist\\BourseViewUSA.xlsx', index=False)
