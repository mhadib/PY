...
class QuotesSpider(scrapy.Spider):
    name = "quotes"
    custom_settings = {
        'LOG_FILE': 'E:/گزارشات لاگ ها/Economist/SoodSherkatHa.txt',
    }

    def start_requests(self):
        urls = [
            'https://fa.tradingeconomics.com/country-list/corporate-profits'
        ]
        for url in urls:
    ...
    def parse(self, response):
        ...
        print(dd)
        dd.to_csv("E:\\PY\Economist\\CSVs\\SoodSherkatHa.csv", encoding='utf-8-sig')
        main()
def main():
    try:
        import logging
        logging.basicConfig(filename='E:/گزارشات لاگ ها/Economist/SoodSherkatHa.txt', level=logging.DEBUG,
        ...
        result = pd.DataFrame()
        finalResult = pd.DataFrame()
        dd = pd.read_csv("E:\\PY\Economist\\CSVs\\SoodSherkatHa.csv")
        ...
        for row in finalResult.itertuples():
            cursor.execute('''INSERT INTO Adib.dbo.SoodSherkatHa([Keshvar], [Gozashte], [Vahed])
                                                       VALUES (?,?,?)
                                                         ''',
                               row[1],
                               row[2],
        ...
        sqlHistory = pd.read_sql_query('select * from vSoodSherkatHa', conn)
        sqlHistory.to_excel('E:\\گزارشات اکسل\\Economist\\vSoodSherkatHa.xlsx', index=False)
