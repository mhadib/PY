...
class QuotesSpider(scrapy.Spider):
    name = "quotes"
    custom_settings = {
        'LOG_FILE': 'E:/گزارشات لاگ ها/Economist/EconomistCurrency.txt',
    }

    def start_requests(self):
        urls = [
            'https://fa.tradingeconomics.com/currencies'
        ]
        for url in urls:
    ...
    def parse(self, response):
        try:
            import logging
            logging.basicConfig(filename='E:/گزارشات لاگ ها/Economist/EconomistCurrency.txt', level=logging.DEBUG,
            ...
            print(dd)
            dd.to_csv("E:\\PY\Economist\\CSVs\\Currency.csv", encoding='utf-8-sig')
            main()
...
def main():
    ...
    result = pd.DataFrame()
    finalResult = pd.DataFrame()
    dd = pd.read_csv("E:\\PY\Economist\\CSVs\\Currency.csv")
    first = True
    for key in pd.read_html(dd['0'][0]):
        ...
        data1.rename(columns={data1.columns[2]: "Rate"}, inplace=True)
        data1 = data1[["Name", "Rate"]]
    ...
    for row in finalResult.itertuples():
        ...
        cursor.execute('''INSERT INTO Adib.dbo.tgjuCurrency([Name] ,[Rate])
                                                                         VALUES ( substring(?,4, 3)+ substring(?,1, 3),1/?)
                                                                           ''',
                       row[1],
                       row[1],
                       row[2]
