...
class QuotesSpider(scrapy.Spider):
    name = "quotes"
    custom_settings = {
        'LOG_FILE': 'E:/گزارشات لاگ ها/Economist/EconomistBedehiKhareji.txt',
    }

    def start_requests(self):
        urls = [
            'https://fa.tradingeconomics.com/country-list/external-debt'
        ]
        for url in urls:
    ...
    def parse(self, response):
        try:
            import logging
            logging.basicConfig(filename='E:/گزارشات لاگ ها/Economist/EconomistBedehiKhareji.txt', level=logging.DEBUG,
            ...
            print(dd)
            dd.to_csv("E:\\PY\Economist\\CSVs\\BedehiKhareji.csv", encoding='utf-8-sig')
            main()
...
def main():
    ...
    dd = pd.read_csv("E:\\PY\Economist\\CSVs\\BedehiKhareji.csv")
    ...
    for row in finalResult.itertuples():
        cursor.execute('''INSERT INTO Adib.dbo.BedehiKhareji([Keshvar] ,[Gozashte],[Vahed])
                                                   VALUES (?,?,?)
                                                     ''',
                           row[1],
                           row[2],
                           row[3])
    conn.commit()
    sqlHistory = pd.read_sql_query('select * from vBedehiKhareji					', conn)
    sqlHistory.to_excel('E:\\گزارشات اکسل\\Economist\\vBedehiKhareji.xlsx', index=False)
    print("main done")
#main()