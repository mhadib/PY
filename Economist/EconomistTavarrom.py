...
class QuotesSpider(scrapy.Spider):
    name = "quotes"
    custom_settings = {
        'LOG_FILE': 'E:/گزارشات لاگ ها/Economist/EconomistTavvarom.txt',
    }
    def start_requests(self):
        urls = [
            'https://fa.tradingeconomics.com/commodities'
        ]
        for url in urls:
    ...
    def parse(self, response):
        try:
            import logging
            logging.basicConfig(filename='E:/گزارشات لاگ ها/Economist/EconomistTavvarom.txt', level=logging.DEBUG,
            ...
            print(dd)
            dd.to_csv("E:\\PY\Economist\\CSVs\\tavarrom.csv", encoding='utf-8-sig')
            dd = pd.read_csv("E:\\PY\Economist\\CSVs\\tavarrom.csv")
            ...
            finalResult = finalResult[['Name', 'قیمت', 'تاریخ', 'ماهیانه', 'YoY', 'هفتگی', '%']]
            ...
            for row in finalResult.itertuples():
                cursor.execute('''INSERT INTO Adib.dbo.Tavarrom([Name] ,['قیمت'] ,['تاریخ'], ['ماهیانه'], [YoY], ['هفتگی'], [%] )
                                                   VALUES (?,?,?,?,?,?,?)
                                                     ''',
                               row[1],
                               row[2],
