...
class QuotesSpider(scrapy.Spider):
    name = "quotes"
    custom_settings = {
        'LOG_FILE': 'E:/گزارشات لاگ ها/Economist/EconomistTolidNaftKhaam.txt',
    }
    def start_requests(self):
        urls = [
            'https://fa.tradingeconomics.com/country-list/crude-oil-production'
        ]
        for url in urls:
    ...
    def parse(self, response):
        try:
            import logging
            logging.basicConfig(filename='E:/گزارشات لاگ ها/Economist/EconomistTolidNaftKhaam.txt', level=logging.DEBUG,
            ...
            print(dd)
            dd.to_csv("E:\\PY\Economist\\CSVs\\TolidNaftKhaam.csv", encoding='utf-8-sig')
            main()
...
def main():
    result = pd.DataFrame()
    finalResult = pd.DataFrame()
    dd = pd.read_csv("E:\\PY\Economist\\CSVs\\TolidNaftKhaam.csv")
    ...
    for row in finalResult.itertuples():
        cursor.execute('''INSERT INTO Adib.dbo.TolidNaftKhaam([Keshvar] ,[Gozashte])
                                                   VALUES (?,?)
                                                     ''',
                           row[1],
                           row[2])
    conn.commit()
    sqlHistory = pd.read_sql_query('select * from vTolidNaftKhaam					', conn)
    sqlHistory.to_excel('E:\\گزارشات اکسل\\Economist\\vTolidNaftKhaam.xlsx', index=False)
    print("main done")
#main()