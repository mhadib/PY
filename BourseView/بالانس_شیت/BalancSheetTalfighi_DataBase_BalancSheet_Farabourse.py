...
logging.basicConfig(filename='E:/گزارشات لاگ ها/BalanceSheet/BalanSheetTalfighi_Balancesheet_Farabourse_Log.txt', level=logging.DEBUG,
...
try:
    ...
    for key, item in StatementHelp['categories'].items():
        ...
        for key2, item2 in dataframe['items'].items():
            ...
            dff1 = pandas.concat([df1, pandas.DataFrame({"item": dataframe2['item'], "helpFA": dataframe2['helpFA']})])
    ...
    finalResult.to_sql("BalanceSheetTalfighiCategurbalancesheet_Farabourse", con=engine, index=False, if_exists='append')

    print("end of write to sql ")

    print("start to write to files ")
    finalResult.to_excel('E:\\گزارشات اکسل\\BourseView\\BalanceSheet\\BalanceSheetTalfighiCategurbalancesheet_Farabourse.xlsx', index=False)
