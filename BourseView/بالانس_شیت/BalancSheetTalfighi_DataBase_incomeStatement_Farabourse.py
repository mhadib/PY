...
logging.basicConfig(filename='E:/گزارشات لاگ ها/BalanceSheet/BalanSheetTalfighi_Incomestatement_Farabourse_Log.txt', level=logging.DEBUG,
...
try:
    ...
    newStock.to_excel('E:\\گزارشات اکسل\\BourseView\\BalanceSheet\\company.xlsx', index=False)
    ...
    df = pandas.DataFrame()
    for i in range(0,20):
        tickersStringIds = helper.get_tickers_farabourse(authToken, i)
        if tickersStringIds == '':
            continue
    ...
    finalResult.to_sql("BalanceSheetTalfighiCategurincomestatement_Farabourse", con=engine, index=False, if_exists='append')
    print("end of write to sql ")

    print("start to write to files ")
    finalResult.to_excel('E:\\گزارشات اکسل\\BourseView\\BalanceSheet\\BalanceSheetTalfighiincomestatement_Farabourse.xlsx', index=False)
